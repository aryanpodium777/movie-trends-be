...
class GenreDao(metaclass=Singleton):
	...
	def fetchGenreByMovieinfoId(self,movieinfoId):
		query = """SELECT id,title FROM `movieinfo_genre` AS m LEFT JOIN `genre` AS g 
					ON m.genre_id = g.id WHERE m.movie_info_id = %s"""
	...
	def fetchGenreByAnalyticsDoughnut(self):
		query = """SELECT `id`,`title`,COUNT(`genre_id`) as count FROM `movieinfo_genre` AS m 
					LEFT JOIN `genre` AS g ON m.genre_id = g.id GROUP BY `genre_id`"""	
		output = self.connection.run(query,False)
		list = []
		for record in output:
			id=record[0]
	...
	def fetchGenreByAnalyticsBar(self,id):
		query = """SELECT YEAR(m.`Released`) as year, SUM( `BoxOffice` ) AS box_office_collection 
				FROM `movieinfo_genre` AS mg LEFT JOIN `movieinfo` AS m ON mg.`movie_info_id` = m.`id` WHERE mg.`genre_id` = %s 
				GROUP BY YEAR(m.`Released`)"""	
